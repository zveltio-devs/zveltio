          ...
          <div class="space-y-3">
            {#each Object.entries(btype.schema) as [key, type]}
              <div class="form-control">
                ...
                {#if type === 'string'}
                  {#if key === 'content'}
                    <textarea bind:value={block.props[key]} class="textarea textarea-bordered textarea-sm" rows="4"></textarea>
                  {:else}
                    <input type="text" bind:value={block.props[key]} class="input input-bordered input-sm" />
                  {/if}
                {:else if type === 'number'}
                  <input type="number" bind:value={block.props[key]} class="input input-bordered input-sm" />
                {/if}
              </div>
            {/each}
          </div>
