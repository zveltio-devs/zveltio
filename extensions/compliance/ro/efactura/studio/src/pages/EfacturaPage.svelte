    ...
    <div class="modal-box w-11/12 max-w-4xl">
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="form-control">
          ...
          <input type="text" bind:value={form.invoice_number} placeholder="FAC-2026-001" class="input input-bordered input-sm" />
        </div>
        <div class="form-control">
          <label class="label"><span class="label-text">Currency</span></label>
          <select bind:value={form.currency} class="select select-bordered select-sm">
            <option>RON</option><option>EUR</option><option>USD</option>
          </select>
        </div>
        <div class="form-control">
          <label class="label"><span class="label-text">Invoice date</span></label>
          <input type="date" bind:value={form.invoice_date} class="input input-bordered input-sm" />
        </div>
        <div class="form-control">
          <label class="label"><span class="label-text">Due date</span></label>
          <input type="date" bind:value={form.due_date} class="input input-bordered input-sm" />
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="card bg-base-200 p-3">
          ...
          <div class="space-y-2">
            <input type="text" bind:value={form.seller_name} placeholder="Company name" class="input input-bordered input-sm w-full" />
            <input type="text" bind:value={form.seller_cui} placeholder="CUI/CIF" class="input input-bordered input-sm w-full font-mono" />
            <input type="text" bind:value={form.seller_address} placeholder="Address" class="input input-bordered input-sm w-full" />
            <input type="text" bind:value={form.seller_iban} placeholder="IBAN" class="input input-bordered input-sm w-full font-mono" />
          </div>
        </div>
        <div class="card bg-base-200 p-3">
          <p class="font-semibold text-sm mb-2">Buyer (Beneficiar)</p>
          <div class="space-y-2">
            <input type="text" bind:value={form.buyer_name} placeholder="Company/person name" class="input input-bordered input-sm w-full" />
            <input type="text" bind:value={form.buyer_cui} placeholder="CUI/CIF (optional)" class="input input-bordered input-sm w-full font-mono" />
            <input type="text" bind:value={form.buyer_address} placeholder="Address" class="input input-bordered input-sm w-full" />
          </div>
        </div>
      </div>
          ...
          <table class="table table-xs">
            ...
            <tbody>
              {#each form.lines as line, i}
                <tr>
                  <td><input type="text" bind:value={line.description} class="input input-bordered input-xs w-40" /></td>
                  <td><input type="number" bind:value={line.quantity} oninput={() => recalcLine(line)} class="input input-bordered input-xs w-16" /></td>
                  <td><input type="text" bind:value={line.unit} class="input input-bordered input-xs w-16" /></td>
                  <td><input type="number" step="0.01" bind:value={line.unit_price} oninput={() => recalcLine(line)} class="input input-bordered input-xs w-24" /></td>
                  <td>
                    <select bind:value={line.vat_rate} onchange={() => recalcLine(line)} class="select select-bordered select-xs w-16">
