    ...
    <div class="modal-box w-11/12 max-w-2xl">
      ...
      <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="form-control">
          ...
          <select bind:value={form.type} class="select select-bordered select-sm">
            {#each DOC_TYPES as t}<option value={t}>{DOC_LABELS[t]}</option>{/each}
          </select>
        </div>
        <div class="form-control">
          <label class="label"><span class="label-text">Număr</span></label>
          <input type="text" bind:value={form.number} placeholder="CT-2026-001" class="input input-bordered input-sm font-mono" />
        </div>
        <div class="form-control">
          <label class="label"><span class="label-text">Data</span></label>
          <input type="date" bind:value={form.date} class="input input-bordered input-sm" />
        </div>
        <div class="form-control col-span-2">
          <label class="label"><span class="label-text">Titlu</span></label>
          <input type="text" bind:value={form.title} placeholder="Contract prestari servicii IT" class="input input-bordered input-sm" />
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        {#each form.parties as party, i}
          <div class="grid grid-cols-3 gap-2 mb-2">
            <input type="text" bind:value={party.name} placeholder="Denumire" class="input input-bordered input-xs" />
            <input type="text" bind:value={party.cui} placeholder="CUI" class="input input-bordered input-xs font-mono" />
            <input type="text" bind:value={party.role} placeholder="Rol" class="input input-bordered input-xs" />
          </div>
        {/each}
      </div>
