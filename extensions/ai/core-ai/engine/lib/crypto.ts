/**
 * Criptare AES-256-GCM pentru chei API AI.
 * Necesită env var: AI_KEY_ENCRYPTION_KEY (32 bytes hex)
 * Fallback la MAIL_ENCRYPTION_KEY dacă AI_KEY_ENCRYPTION_KEY nu e setat.
 */
...
export async function encryptApiKey(plaintext: string): Promise<string> {
  ...
  const keyMaterial = await crypto.subtle.importKey(
    'raw', Buffer.from(getKey().slice(0, 64), 'hex'),
    { name: 'AES-GCM' }, false, ['encrypt'],
  );
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const encrypted = await crypto.subtle.encrypt(
    { name: 'AES-GCM', iv }, keyMaterial, new TextEncoder().encode(plaintext),
  );
...
export async function decryptApiKey(stored: string): Promise<string> {
  ...
  const decrypted = await crypto.subtle.decrypt(
    ...
    keyMaterial, Buffer.from(cipherHex, 'hex'),
  );
