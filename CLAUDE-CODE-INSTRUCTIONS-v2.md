# CLAUDE CODE INSTRUCTIONS — Zveltio Migration & Architecture Fix
...
[ ] FAZA 1   — WebSocket + Realtime cu pg LISTEN/NOTIFY
[ ] FAZA 2   — Flow Executor conectat la Scheduler, routes CRUD + migrare
[ ] FAZA 3   — Multi-Tenancy cu PostgreSQL RLS (SET LOCAL, nu WHERE manual)
[ ] FAZA 4   — Virtual Collections cu Query Translator (nu fetch-all!)
[ ] FAZA 5   — Ghost DDL pentru zero-downtime schema changes (>100k rows)
[ ] FAZA 6   — 14 rute API portate din old repo + migrări
[ ] FAZA 7   — AI complet (core-ai + Z-AI + analytics)
[ ] FAZA 8   — Studio: 10 componente + 6 pagini + nav links
...
[ ] FAZA 11  — SDK Local-First: IndexedDB store + SyncManager + /api/sync endpoint
[ ] SMOKE TEST — bun install, bun run build, tsc --noEmit, build:binary, curl health
```

**TIMP ESTIMAT: 18-23 zile de lucru concentrat.**
