...
- Community-tier extensions are **review-gated and signed**, and their request handling runs in a worker: the host restricts their SQL to user-data tables and their own `zv_<ext>_*` namespace, on a reserved connection with a statement timeout, and the worker starts with a minimal environment so engine credentials are not visible to it. **This is not full isolation yet** — an extension's top-level module code is imported in the engine process before its worker is spawned, so it runs with engine privileges once, at load. Treat "worker-isolated" as a defence-in-depth layer over review and signing, not as a boundary that contains hostile code. See `docs/SECURITY.md`.
