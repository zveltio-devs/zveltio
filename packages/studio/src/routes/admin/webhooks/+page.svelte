    ...
    <div class="modal-box w-11/12 max-w-2xl">
      ...
      <div class="space-y-4">
        <div class="form-control">
          ...
          <input class="input input-bordered" bind:value={form.name} placeholder="My Webhook" />
        </div>
        <div class="form-control">
          <label class="label"><span class="label-text">URL *</span></label>
          <input class="input input-bordered font-mono" bind:value={form.url} placeholder="https://example.com/webhook" />
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Method</span></label>
            <select class="select select-bordered" bind:value={form.method}>
              <option>POST</option><option>PUT</option><option>PATCH</option>
            </select>
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">Secret (optional)</span></label>
            <input class="input input-bordered font-mono" bind:value={form.secret} placeholder="Signing secret" />
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Retry attempts</span></label>
            <input type="number" class="input input-bordered" bind:value={form.retry_attempts} min="0" max="10" />
          </div>
          <div class="form-control">
            <label class="label"><span class="label-text">Timeout (ms)</span></label>
            <input type="number" class="input input-bordered" bind:value={form.timeout} min="1000" max="30000" step="500" />
          </div>
        </div>
        <div class="form-control">
