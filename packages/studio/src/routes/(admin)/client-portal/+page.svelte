<script lang="ts">
  ...
  import {
    ...
    FileCheck, ShieldCheck, Eye, ExternalLink, ToggleLeft, ToggleRight,
  } from '@lucide/svelte';
  ...
  const TEMPLATES = [
    { value: 'generic',     label: 'Generic',     desc: 'Portal simplu cu tichete de suport și profil' },
    { value: 'saas',        label: 'SaaS',         desc: 'Dashboard abonament, suport, cont' },
    { value: 'services',    label: 'Servicii',     desc: 'Portal pentru firme de servicii profesionale' },
    { value: 'regulatory',  label: 'Instituțional', desc: 'Autorizații, controale, puncte de lucru (operatori economici)' },
  ];
...
<div class="max-w-3xl">
  <div class="mb-6">
    ...
    <p class="text-sm text-base-content/50 mt-0.5">Configurează portalul de clienți al organizației</p>
  </div>

  <!-- Tabs -->
  <div class="tabs tabs-bordered mb-6">
    <button class="tab {tab === 'config' ? 'tab-active' : ''}" onclick={() => (tab = 'config')}>Configurare</button>
    <button class="tab {tab === 'operators' ? 'tab-active' : ''}" onclick={() => (tab = 'operators')}>Operatori</button>
  </div>
  ...
  {:else if tab === 'config'}
    ...
    {#if config.is_enabled}
      <div class="alert bg-success/10 border border-success/20 text-sm mb-6">
        ...
        <span class="text-base-content/70">Portalul este activ.</span>
        <a href="{base}/portal-client/login" target="_blank"
          class="btn btn-xs btn-success gap-1 ml-auto">
    ...
    {:else}
      <div class="alert bg-base-200 border border-base-300 text-sm mb-6">
        ...
        <span class="text-base-content/50">Portalul este dezactivat. Activează-l din opțiunile de mai jos.</span>
      </div>
    {/if}
    ...
    {/if}
    {#if success}
      <div class="alert alert-success text-sm py-2 mb-4"><CheckCircle size={14} /><span>Configurare salvată.</span></div>
    {/if}
    ...
    <div class="space-y-5">
      ...
      <div class="card bg-base-200 border border-base-300">
        <div class="card-body p-4 flex-row items-center justify-between">
          <div>
            <p class="font-semibold text-sm">Activare portal</p>
            <p class="text-xs text-base-content/50">Clientii pot accesa portalul la <code class="bg-base-300 px-1 rounded">/portal-client/login</code></p>
          </div>
          <button onclick={() => (config.is_enabled = !config.is_enabled)}
            class="text-{config.is_enabled ? 'primary' : 'base-content/30'} transition-colors">
            {#if config.is_enabled}
      ...
      <!-- Template -->
      <div>
        <p class="text-xs font-semibold text-base-content/50 uppercase tracking-wide mb-3">Template portal</p>
      ...
      <div class="card bg-base-200 border border-base-300">
        <div class="card-body p-5 gap-4">
          <p class="font-semibold text-sm">Identitate vizuală</p>

          <div class="grid sm:grid-cols-2 gap-4">
            <div class="form-control gap-1">
              <label class="label py-0"><span class="label-text text-xs font-medium">Denumire portal</span></label>
              <input type="text" bind:value={config.site_name} class="input input-sm" placeholder="Client Portal" />
            </div>
            <div class="form-control gap-1">
              <label class="label py-0"><span class="label-text text-xs font-medium">Culoare principală</span></label>
            ...
            <div class="form-control gap-1 sm:col-span-2">
              <label class="label py-0"><span class="label-text text-xs font-medium">URL logo (opțional)</span></label>
          ...
          <!-- Live preview -->
          <div class="mt-2">
            <p class="text-xs text-base-content/40 mb-2">Previzualizare header portal:</p>
            <div class="flex items-center gap-3 bg-base-100 rounded-xl px-4 py-3 border border-base-300">
              ...
              <div>
                ...
                <p class="text-[11px] text-base-content/40 mt-0.5 capitalize">{config.template} portal</p>
              </div>
            </div>
          </div>
        </div>
    ...
    {:else if operators.length === 0}
      <div class="card bg-base-200 border border-base-300">
        <div class="card-body items-center text-center py-16">
          ...
          <p class="font-medium text-sm text-base-content/60">Niciun operator înregistrat</p>
          <p class="text-xs text-base-content/40 mt-1">Operatorii economici care s-au înregistrat în portal vor apărea aici.</p>
        </div>
      </div>
    {:else}
            ...
            <div class="card-body p-4 gap-3">
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p class="text-xs text-base-content/50">CUI: {op.fiscal_code}{op.legal_form ? ` · ${op.legal_form}` : ''}</p>
                  {#if op.address}
              ...
              {#if op.users?.length}
                <div>
                  <p class="text-xs font-semibold text-base-content/40 uppercase tracking-wide mb-2">Utilizatori asociați</p>
                  <div class="space-y-1.5">
                    {#each op.users as u}
                      <div class="flex items-center justify-between bg-base-100 rounded-lg px-3 py-2">
                        ...
                        {:else}
                          <button class="btn btn-xs btn-warning gap-1"
                            onclick={() => verifyUser(op.id, u.user_id)}>
                            <ShieldCheck size={11} /> Verifică
                          </button>
                        {/if}
                      </div>
                    {/each}
