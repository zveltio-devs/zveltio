...
async function loadPositions(): Promise<void> {
  ...
  try {
    const res = await api.get<{ positions: Record<string, { x: number; y: number }> }>(
      '/api/erd/layout',
    );
    ...
    /* offline or pre-migration engine — stay on local */
  }
}
...
async function resetLayout() {
  ...
  try {
    await api.delete('/api/erd/layout');
  } catch {
    /* ignore offline */
  }
...
function buildExportSvg(): string {
  ...
  lines.push(
    '<style>.t{font:600 12px Inter,sans-serif;fill:#0f172a}.tn{font:11px monospace;fill:#475569}.tf{font:11px monospace;fill:#1e293b}.tr{font:11px monospace;fill:#4f46e5}.ty{font:10px monospace;fill:#94a3b8}</style>',
  );
  ...
  for (const c of visible) {
    ...
    lines.push(
      `<rect width="${NODE_W}" height="${totalH}" rx="8" ry="8" fill="#ffffff" stroke="#cbd5e1" stroke-width="2"/>`,
    );
    ...
    shown.forEach((f, i) => {
      ...
      lines.push(
        `<text class="ty" x="${NODE_W - 12}" y="${y}" text-anchor="end">${escape(f.type)}</text>`,
      );
    });
...
function edgeColor(type: string): string {
  switch (type) {
    case 'm2o':
    case 'reference':
      return '#6366f1'; // indigo — N→1
    case 'o2m':
      return '#22c55e'; // green  — 1→N
    case 'm2m':
      return '#f59e0b'; // amber  — N↔N
    default:
      return '#94a3b8';
  }
}
