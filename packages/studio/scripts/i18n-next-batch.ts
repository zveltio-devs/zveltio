...
const KEYS: Record<string, { en: string; ro: string }> = {
  ...
  'ext.confirm.deleteForm': {
    en: 'Delete form "{name}"? This will also delete all submissions.',
    ro: 'Ștergi formularul "{name}"? Se vor șterge și toate trimiterile.',
  },
  'ext.confirm.removeSearchIndex': {
    en: 'Remove search index for "{collection}"?',
    ro: 'Elimini indexul de căutare pentru "{collection}"?',
  },
...
patch('hr/time-tracking/studio/pages/+page.svelte', [
  ...
  [
    ...
    '<span class="label-text text-xs">{m[\'hr.time-tracking.ui.description\']()}</span>',
  ],
  [
    '<span class="label-text text-xs">Client</span>',
    '<span class="label-text text-xs">{m[\'common.col.client\']()}</span>',
  ],
  [
    '<span class="label-text text-xs">Currency</span>',
    '<span class="label-text text-xs">{m[\'common.col.currency\']()}</span>',
  ],
...
patch('finance/accounting/studio/pages/+page.svelte', [
  ...
  [
    '<BookOpen size={13} class="mr-1.5" /> Journal entries',
    '<BookOpen size={13} class="mr-1.5" /> {m[\'finance.accounting.tab.entries\']()}',
  ],
  [
    '<Coins size={13} class="mr-1.5" /> Chart of accounts',
    '<Coins size={13} class="mr-1.5" /> {m[\'finance.accounting.tab.accounts\']()}',
  ],
  [
    '<TrendingUp size={13} class="mr-1.5" /> Fiscal years',
    '<TrendingUp size={13} class="mr-1.5" /> {m[\'finance.accounting.tab.fiscal\']()}',
  ],
  [
  ...
  [
    ...
    '<span class="label-text text-xs">{m[\'common.col.name\']()}</span>',
  ],
  [
  ...
  [
    "{f.is_closed ? 'closed' : 'open'}",
    "{f.is_closed ? m['finance.accounting.status.closed']() : m['finance.accounting.status.open']()}",
  ],
  ['{/if} Create', "{/if}{m['common.create']()}"],
]);

// Add missing key if used
en['finance.accounting.ui.no_journal_entries'] =
  en['finance.accounting.ui.no_journal_entries'] ?? 'No journal entries.';
ro['finance.accounting.ui.no_journal_entries'] =
  ro['finance.accounting.ui.no_journal_entries'] ?? 'Nicio înregistrare în jurnal.';
...
patch('crm/studio/pages/contacts/+page.svelte', [
  ...
  [
    ...
    "{editingContact ? m['crm.ui.editContact']() : m['crm.ui.newContactTitle']()}",
  ],
  ['>Prev</button>', ">{m['common.prev']()}</button>"],
  ['>Next</button>', ">{m['common.next']()}</button>"],
  [
    'Page {page} of {Math.ceil(total / 20) || 1}',
    "{m['common.pageOf']({ page: String(page), total: String(Math.ceil(total / 20) || 1) })}",
  ],
...
patch('search/studio/pages/+page.svelte', [
  [
    'if (!confirm(`Remove search index for "${collection}"?`))',
    "if (!confirm(m['ext.confirm.removeSearchIndex']({ collection })))",
  ],
]);
...
patch('storage/cloud/studio/pages/+page.svelte', [
  [
    ...
    "if (!confirm(m['ext.confirm.deleteNamed']({ name: e.name })))",
  ],
]);
...
patch('forms/studio/pages/+page.svelte', [
  [
    ...
    "if (!confirm(m['ext.confirm.deleteForm']({ name })))",
  ],
]);
