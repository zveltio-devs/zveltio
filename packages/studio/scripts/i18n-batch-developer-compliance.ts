...
const KEYS: Record<string, { en: string; ro: string }> = {
  ...
  'compliance.ro.efactura.section.buyer': { en: 'Buyer (Beneficiary)', ro: 'Cumpărător (Beneficiar)' },
  ...
  'compliance.ro.efactura.toast.submissionFailed': { en: 'Submission failed', ro: 'Trimiterea a eșuat' },
  // e-Transport
  'compliance.ro.etransport.btn.newDeclaration': { en: 'New Declaration', ro: 'Declarație nouă' },
  'compliance.ro.etransport.btn.createDeclaration': { en: 'Create Declaration', ro: 'Creează declarația' },
  'compliance.ro.etransport.empty.declarations': { en: 'No transport declarations', ro: 'Nicio declarație de transport' },
  ...
  'developer.byod.form.includePatterns': { en: 'Include patterns (comma-separated)', ro: 'Pattern-uri incluse (separate prin virgulă)' },
  ...
  'developer.graphql.placeholder.output': { en: '(run a query to see output)', ro: '(rulează o interogare pentru rezultat)' },
  ...
  'developer.api-docs.link.openApi': { en: 'View OpenAPI spec →', ro: 'Vezi specificația OpenAPI →' },
  'developer.api-docs.empty.changelog': { en: 'No changelog entries.', ro: 'Nicio intrare în jurnal.' },
  ...
  'developer.database.selectTable': { en: 'Select a table to inspect.', ro: 'Selectează o tabelă pentru inspecție.' },
  ...
  'developer.views.error.invalidJson': { en: 'Invalid JSON in config', ro: 'JSON invalid în config' },
};
...
injectFn('compliance/ro/efactura/studio/pages/+page.svelte', '  function statusBadge(status: string): string {', efacturaStatusFn);
injectFn('compliance/ro/etransport/studio/pages/+page.svelte', '  function statusBadge(status: string): string {', etransportStatusFn);

patch('compliance/ro/efactura/studio/pages/+page.svelte', [
  ['<Plus size={14} /> New Invoice</button>', '<Plus size={14} /> {m[\'compliance.ro.efactura.btn.newInvoice\']()}</button>'],
  ['{s === \'all\' ? m[\'common.filter.all\']() : s.replace(\'_\', \' \')}', '{invoiceStatusLabel(s)}'],
  ['<FileText size={40} class="opacity-20 mb-2" /> No invoices', '<FileText size={40} class="opacity-20 mb-2" /> {m[\'compliance.ro.efactura.empty.invoices\']()}'],
  ['<th>Buyer</th>', '<th>{m[\'compliance.ro.efactura.col.buyer\']()}</th>'],
  ['<th>ANAF Index</th>', '<th>{m[\'compliance.ro.efactura.col.anafIndex\']()}</th>'],
  ['<p class="font-semibold text-sm mb-2">Seller (Emitent)</p>', '<p class="font-semibold text-sm mb-2">{m[\'compliance.ro.efactura.section.seller\']()}</p>'],
  ['<p class="font-semibold text-sm mb-2">Buyer (Beneficiar)</p>', '<p class="font-semibold text-sm mb-2">{m[\'compliance.ro.efactura.section.buyer\']()}</p>'],
  ['<p class="font-semibold text-sm">Invoice lines</p>', '<p class="font-semibold text-sm">{m[\'compliance.ro.efactura.section.lines\']()}</p>'],
  ...
  ['<p class="text-base font-bold">Total:', '<p class="text-base font-bold">{m[\'compliance.ro.efactura.totals.total\']()}:'],
  [' Create Invoice', " {m['compliance.ro.efactura.btn.createInvoice']()}"],
  ["toast.error(e?.message ?? 'Submission failed')", "toast.error(e?.message ?? m['compliance.ro.efactura.toast.submissionFailed']())"],
]);

patch('compliance/ro/etransport/studio/pages/+page.svelte', [
  ['<Plus size={14} /> New Declaration</button>', '<Plus size={14} /> {m[\'compliance.ro.etransport.btn.newDeclaration\']()}</button>'],
  ['{s === \'all\' ? m[\'common.filter.all\']() : s.replace(\'_\', \' \')}', '{transportStatusLabel(s)}'],
  ['<Truck size={40} class="opacity-20 mb-2" /> No transport declarations', '<Truck size={40} class="opacity-20 mb-2" /> {m[\'compliance.ro.etransport.empty.declarations\']()}'],
  ...
  ['<p class="font-semibold text-sm mb-2">Departure</p>', '<p class="font-semibold text-sm mb-2">{m[\'compliance.ro.etransport.section.departure\']()}</p>'],
  ['<p class="font-semibold text-sm mb-2">Destination</p>', '<p class="font-semibold text-sm mb-2">{m[\'compliance.ro.etransport.section.destination\']()}</p>'],
  ['<p class="font-semibold text-sm">Goods</p>', '<p class="font-semibold text-sm">{m[\'compliance.ro.etransport.section.goods\']()}</p>'],
...
patch('developer/byod/studio/pages/+page.svelte', [
  ['Connect to an external PostgreSQL database, scan its schema, and surface tables as virtual collections in Zveltio.', "{m['developer.byod.intro']()}"],
  ...
  ["{scanning === p.id ? 'Scanning…' : 'Scan'}", "{scanning === p.id ? m['developer.byod.btn.scanning']() : m['developer.byod.btn.scan']()}"],
  ['<label class="label label-text">Name</label>', '<label class="label label-text">{m[\'developer.byod.form.name\']()}</label>'],
  ['<label class="label label-text">Connection string</label>', '<label class="label label-text">{m[\'developer.byod.form.connectionString\']()}</label>'],
  ['<label class="label label-text">Schema</label>', '<label class="label label-text">{m[\'developer.byod.form.schema\']()}</label>'],
  ['<label class="label label-text">Include patterns (comma-separated)</label>', '<label class="label label-text">{m[\'developer.byod.form.includePatterns\']()}</label>'],
  ['<label class="label label-text">Exclude patterns</label>', '<label class="label label-text">{m[\'developer.byod.form.excludePatterns\']()}</label>'],
  ["{saving ? 'Saving…' : m['common.create']()}", "{saving ? m['developer.byod.saving']() : m['common.create']()}"],
]);

patch('developer/graphql/studio/pages/+page.svelte', [
  ['<Play size={13} class="mr-1.5" /> Playground</button>', '<Play size={13} class="mr-1.5" /> {m[\'developer.graphql.tab.playground\']()}</button>'],
  ['>Operation logs</button>', ">{m['developer.graphql.tab.logs']()}</button>"],
  ['<Save size={13} class="mr-1.5" /> Persisted queries</button>', '<Save size={13} class="mr-1.5" /> {m[\'developer.graphql.tab.persisted\']()}</button>'],
  ['>Field policies</button>', ">{m['developer.graphql.tab.policies']()}</button>"],
  ['<span class="font-medium text-sm">Query</span>', '<span class="font-medium text-sm">{m[\'developer.graphql.panel.query\']()}</span>'],
  ['<div class="p-3 border-b border-base-300 font-medium text-sm">Response</div>', '<div class="p-3 border-b border-base-300 font-medium text-sm">{m[\'developer.graphql.panel.response\']()}</div>'],
  ...
  ["{l.error ? 'error' : 'ok'}", "{l.error ? m['developer.graphql.status.error']() : m['developer.graphql.status.ok']()}"],
  ["toast.error(e?.message ?? 'Query failed')", "toast.error(e?.message ?? m['developer.graphql.error.queryFailed']())"],
]);
...
patch('developer/validation/studio/pages/+page.svelte', [
  ['<span class="label-text text-xs">Description</span>', '<span class="label-text text-xs">{m[\'developer.validation.form.description\']()}</span>'],
]);

patch('developer/database/studio/pages/+page.svelte', [
  ['<div class="font-medium text-sm mb-2">Tables ({tables.length})</div>', '<div class="font-medium text-sm mb-2">{m[\'developer.database.tables\']()} ({tables.length})</div>'],
  ['<li class="p-3 text-base-content/60 text-sm">Loading…</li>', '<li class="p-3 text-base-content/60 text-sm">{m[\'developer.database.loading\']()}</li>'],
  ['<li class="p-3 text-base-content/60 text-sm">No tables.</li>', '<li class="p-3 text-base-content/60 text-sm">{m[\'developer.database.noTables\']()}</li>'],
  ['<p class="text-base-content/50 text-sm">Select a table to inspect.</p>', '<p class="text-base-content/50 text-sm">{m[\'developer.database.selectTable\']()}</p>'],
  ['{rowCount.toLocaleString()} rows · {columns.length} columns', '{rowCount.toLocaleString()} {m[\'developer.database.rows\']()} · {columns.length} {m[\'developer.database.columns\']()}'],
  ['uppercase tracking-wider">Schema</div>', 'uppercase tracking-wider">{m[\'developer.database.section.schema\']()}</div>'],
  ...
  ['uppercase tracking-wider">Sample rows</div>', 'uppercase tracking-wider">{m[\'developer.database.section.sampleRows\']()}</div>'],
  ['text-center py-6 text-base-content/50 text-sm">No rows.</td>', 'text-center py-6 text-base-content/50 text-sm">{m[\'developer.database.noRows\']()}</td>'],
  ["? 'yes' : 'no'", "? m['developer.database.yes']() : m['developer.database.no']()"],
]);

patch('developer/views/studio/pages/+page.svelte', [
  ['<Plus size={14} /> New view</button>', '<Plus size={14} /> {m[\'developer.views.btn.newView\']()}</button>'],
  ['No saved views yet.', "{m['developer.views.empty']()}"],
  ["throw new Error('Invalid JSON in config')", "throw new Error(m['developer.views.error.invalidJson']())"],
]);
