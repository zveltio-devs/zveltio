...
const KEYS: Record<string, { en: string; ro: string }> = {
  ...
  'developer.validation.subtitle': { en: 'Field-level validation with AI assistance', ro: 'Validare la nivel de câmp cu asistență AI' },
  ...
  'developer.validation.aiPlaceholder': { en: "e.g. 'Romanian CNP - 13 digits'", ro: "ex. 'CNP românesc - 13 cifre'" },
  'developer.validation.aiRequires': { en: 'Requires the AI extension to be active.', ro: 'Necesită extensia AI activă.' },
  ...
  'developer.validation.error.invalidJson': { en: 'Invalid JSON in config', ro: 'JSON invalid în config' },
  ...
  'forms.empty': { en: 'No forms yet. Use the Forms API to create embeddable forms.', ro: 'Nicio formular încă. Folosește API Forms pentru formulare embed.' },
  ...
  'operations.traceability.dispatches.directSuccess': { en: 'Dispatch recorded for', ro: 'Expediere înregistrată pentru' },
  ...
  'ai.search.queryPlaceholder': { en: 'e.g. articles about machine learning', ro: 'ex. articole despre machine learning' },
  'ai.query.placeholder': { en: 'e.g. show me the 10 most recent users', ro: 'ex. ultimii 10 utilizatori înregistrați' },
  'ai.schema.placeholder': { en: 'e.g. a blog with posts, title, content, status', ro: 'ex. blog cu postări, titlu, conținut, status' },
};
...
function fixCorruptOptions(c: string): string {
  ...
  return c.replace(/value="\{m\['[^"]+\{m\[[^\]]+\][^"]*"\]\(\)\}[^"]*">([^<]+)<\/option>/g, (_, label) => {
...
patch('operations/pos/studio/pages/+page.svelte', [
  ...
  ['<Receipt size={14} /> Recent orders', '<Receipt size={14} /> {m[\'operations.pos.recentOrders\']()}'],
  ['<th>Time</th>', '<th>{m[\'operations.pos.col.time\']()}</th>'],
  ['Recent Z-reports', "{m['operations.pos.zReports']()}"],
  ['<th class="text-right">Sales</th>', '<th class="text-right">{m[\'operations.pos.col.sales\']()}</th>'],
  ['<th class="text-right">Orders</th>', '<th class="text-right">{m[\'operations.pos.col.orderCount\']()}</th>'],
]);
...
patch('storage/cloud/studio/pages/+page.svelte', [
  ...
  ['<th class="text-right">Size</th>', '<th class="text-right">{m[\'storage.cloud.col.size\']()}</th>'],
...
patch('crm/studio/pages/transactions/+page.svelte', [
  ['<h1 class="text-2xl font-bold">Transactions</h1>', '<h1 class="text-2xl font-bold">{m[\'crm.transactions.title\']()}</h1>'],
...
patch('crm/studio/pages/organizations/+page.svelte', [
  ['<h1 class="text-2xl font-bold">Organizations</h1>', '<h1 class="text-2xl font-bold">{m[\'crm.organizations.title\']()}</h1>'],
...
patch('operations/traceability/studio/pages/dispatches/+page.svelte', [
  ['Expediere înregistrată cu succes pentru', "{m['operations.traceability.dispatches.directSuccess']()}"],
  ['onclick={() => {m[\'operations.traceability.ui.directdone_null_alta\']()}', 'onclick={() => directDone = null}>{m[\'operations.traceability.dispatches.another\']()}'],
  ["onclick={assignLot}>{m['operations.traceability.ui.asigneaz']()}", 'onclick={assignLot}>{m[\'common.assign\']()}'],
]);
...
const valPatches: [RegExp | string, string][] = [
  ["toast.error(e?.message ?? 'Failed to load')", "toast.error(e?.message ?? m['ext.loadFailed']())"],
  ["throw new Error('Invalid JSON in config')", "throw new Error(m['developer.validation.error.invalidJson']())"],
  ...
  ["toast.error(e?.message ?? 'AI generation failed')", "toast.error(e?.message ?? m['developer.validation.error.aiFailed']())"],
  ...
  ["placeholder=\"e.g. 'Romanian CNP - 13 digits'\"", 'placeholder={m[\'developer.validation.aiPlaceholder\']()}'],
...
try { unlinkSync(valBackup); } catch {}
}
...
for (const p of walkSvelte(EXT)) {
  ...
  if (n !== c) { writeFileSync(p, n); fixedOpts++; }
}
