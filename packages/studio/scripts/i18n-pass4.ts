...
const KEYS: [string, string, string][] = [
  ...
  ['geospatial.postgis.cluster.returnsHint', 'Returns clusters with centroid coordinates and point counts.', 'Returnează clustere cu coordonate centroid și număr de puncte.'],
  ...
  ['content.media.confirm.deleteFilesMsg', 'Delete {n} selected files?', 'Ștergi {n} fișiere selectate?'],
  ...
  ['data.import.label.upsertHtml', 'Upsert on field (optional, e.g. email)', 'Upsert pe câmp (opțional, ex. email)'],
  ['developer.edge-functions.shell.title', 'Edge Functions', 'Funcții Edge'],
  ['developer.edge-functions.shell.subtitle', 'Deploy and test serverless functions', 'Deploy și testează funcții serverless'],
  ['communications.mail.shell.useLayout', 'Full-height mail client', 'Client mail pe înălțime completă'],
];
...
const P: Record<string, [string, string][]> = {
  'geospatial/postgis/studio/pages/+page.svelte': [
    ...
    ['>Geofences ({geofences.length})</button>', ">{m['geospatial.postgis.tab.geofencesCount']({ n: String(geofences.length) })}</button>"],
    ['>Clustering</button>', ">{m['geospatial.postgis.tab.clustering']()}</button>"],
    ['{/if} Search\n', "{/if}{m['geospatial.postgis.btn.search']()}\n"],
    ['{nearResults.length} results</p>', "{m['geospatial.postgis.resultsCount']({ n: String(nearResults.length) })}</p>"],
    ['{Math.round(r.distance_meters)}m away</span>', "{m['geospatial.postgis.distanceMeters']({ d: String(Math.round(r.distance_meters)) })}</span>"],
    ['{/if} Create\n', "{/if}{m['geospatial.postgis.btn.create']()}\n"],
    ['Use the API endpoint', "{m['geospatial.postgis.cluster.apiHint']()}"],
    ['Returns clusters with centroid coordinates and point counts.', "{m['geospatial.postgis.cluster.returnsHint']()}"],
  ],
  ...
  'content/media/studio/pages/+page.svelte': [
    ...
    ['`Delete ${selectedFiles.size} selected files?`', "m['content.media.confirm.deleteFilesMsg']({ n: String(selectedFiles.size) })"],
    ['<Trash2 size={14} /> Delete ({selectedFiles.size})', "<Trash2 size={14} /> {m['content.media.btn.deleteSelected']({ n: String(selectedFiles.size) })"],
    ['>Download</a>', ">{m['content.media.btn.download']()}</a>"],
    ['<Download size={16} /> Download', "<Download size={16} /> {m['content.media.btn.download']()}"],
    ['<Trash2 size={16} /> Delete', "<Trash2 size={16} /> {m['common.delete']()}"],
  ],
  'data/export/studio/pages/+page.svelte': [
    ['<Download class="h-4 w-4" /> Download</button>', "<Download class=\"h-4 w-4\" /> {m['data.export.btn.download']()}</button>"],
  ],
  'data/import/studio/pages/+page.svelte': [
    ['<label class="label label-text">Format</label>', "<label class=\"label label-text\">{m['data.import.label.format']()}</label>"],
    ['Upsert on field (optional, e.g. <code class="text-xs">email</code>)', "{m['data.import.label.upsertHtml']()}"],
    ['<div class="font-medium">Imported {result.rows_imported ?? \'?\'} rows</div>', "<div class=\"font-medium\">{m['data.import.toast.imported']({ n: String(result.rows_imported ?? '?') })}</div>"],
    ['{result.errors.length} errors — see job log</div>', "{m['data.import.errorsHint']({ n: String(result.errors.length) })}</div>"],
  ],
  ...
  'hr/payroll/studio/pages/+page.svelte': [
    ...
    ['<Download size={13} /> Revisal XML', "<Download size={13} /> {m['hr.payroll.btn.revisal']()}"],
  ...
  'integrations/api-connector/studio/pages/+page.svelte': [
    ['<Plus size={14} /> New connection', "<Plus size={14} /> {m['integrations.api-connector.btn.new']()}"],
  ...
  'storage/cloud/studio/pages/+page.svelte': [
    ['<h3 class="font-semibold">Share {selected?.name}</h3>', "<h3 class=\"font-semibold\">{m['storage.cloud.shareTitle']({ name: selected?.name ?? '' })}</h3>"],
    ['>Share\n', ">{m['storage.cloud.btn.share']()}\n"],
  ],
  'workflow/approvals/studio/pages/+page.svelte': [
    ['<Workflow size={13} class="mr-1.5" /> Workflows', "<Workflow size={13} class=\"mr-1.5\" /> {m['workflow.approvals.tab.workflowsLabel']()}"],
  ],
  'workflow/checklists/studio/pages/+page.svelte': [
    ['\n            Create\n', "\n            {m['workflow.checklists.btn.create']()}\n"],
    ['<BarChart2 size={13}/> Responses', "<BarChart2 size={13}/> {m['workflow.checklists.tab.responses']()}"],
    ['Edit <ChevronRight', "{m['workflow.checklists.btn.edit']()} <ChevronRight"],
    ['{(c.items ?? []).length} item{(c.items ?? []).length !== 1 ? \'s\' : \'\'}', "{m['workflow.checklists.itemsCount']({ n: String((c.items ?? []).length) })"],
  ...
  'developer/edge-functions/studio/pages/+page.svelte': [
    [': {m[\'developer.edge-functions.status.inactive\']()}', ": m['developer.edge-functions.status.inactive']()"],
  ],
};
