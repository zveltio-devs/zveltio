...
const KEYS: Record<string, { en: string; ro: string }> = {
  ...
  'developer.edge-functions.empty.selectOrCreate': { en: 'Select a function or create one', ro: 'Selectează o funcție sau creează una' },
  ...
  'developer.edge-functions.form.mountedAt': { en: 'Mounted at /api/fn/', ro: 'Montat la /api/fn/' },
  ...
  'ai.studio.noProvider': { en: 'No AI provider configured.', ro: 'Niciun furnizor AI configurat.' },
  ...
  'ai.chat.startConversation': { en: 'Send a message to start the conversation', ro: 'Trimite un mesaj pentru a începe conversația' },
  ...
  'ai.query.hint': { en: 'Ask in natural language — get SQL + results.', ro: 'Întreabă în limbaj natural — primești SQL + rezultate.' },
  ...
  'ai.schema.hint': { en: 'Describe your data model — AI generates the schema.', ro: 'Descrie modelul de date — AI generează schema.' },
  ...
  'ai.prompt.collectionsCounts': { en: 'Show me all collections with their record counts', ro: 'Arată toate colecțiile cu numărul de înregistrări' },
  'ai.prompt.ecommerceSchema': { en: 'Generate a schema for an e-commerce product catalog', ro: 'Generează o schemă pentru catalog produse e-commerce' },
  'ai.prompt.pendingRecords': { en: 'Find records where status is pending from the last 7 days', ro: 'Găsește înregistrări cu status pending din ultimele 7 zile' },
  'ai.prompt.activeCollections': { en: 'What are the most active collections this week?', ro: 'Care sunt cele mai active colecții săptămâna aceasta?' },
};
...
const AI_KEY_RENAMES: [string, string][] = [
  ...
  ["m['ai.ui.type_a_message_enter_to_send_shift_enter_for_new']", "m['ai.chat.messagePlaceholder']"],
...
patch('developer/edge-functions/studio/pages/+page.svelte', [
  ['<span class="font-semibold text-sm">Edge Functions</span>', `<span class="font-semibold text-sm">{${EF}.panel.title']()}</span>`],
  ['<Code size={28} class="mx-auto mb-2 opacity-30" /> No functions', `<Code size={28} class="mx-auto mb-2 opacity-30" /> {${EF}.empty.functions']()}`],
  ["'Inactive'", `{${EF}.status.inactive']()}`],
  ['{/if} Save', `{/if}{${EF}.btn.save']()}`],
  ['{/if} Run', `{/if}{${EF}.btn.run']()}`],
  ['<div class="p-3 border-b border-base-300 font-medium text-sm">Test Invoke</div>', `<div class="p-3 border-b border-base-300 font-medium text-sm">{${EF}.test.title']()}</div>`],
  ['<span class="text-xs font-medium">Response</span>', `<span class="text-xs font-medium">{${EF}.test.response']()}</span>`],
  ['<p class="text-xs font-medium mb-0.5">Console</p>', `<p class="text-xs font-medium mb-0.5">{${EF}.test.console']()}</p>`],
  ['<p class="text-sm">Select a function or create one</p>', `<p class="text-sm">{${EF}.empty.selectOrCreate']()}</p>`],
  ['<Plus size={14} /> New Function</button>', `<Plus size={14} /> {${EF}.btn.newFunction']()}</button>`],
  ...
  ["toast.error(e?.message ?? 'Invoke failed')", `toast.error(e?.message ?? ${EF}.error.invoke']())`],
]);

patch('ai/studio/pages/+page.svelte', [
  ...AI_KEY_RENAMES,
  ['<Plus size={14} /> New Chat</button>', '<Plus size={14} /> {m[\'ai.action.newChat\']()}</button>'],
  ...
  ['<p class="text-xs font-semibold text-base-content/60 uppercase mb-2">AI Providers</p>', '<p class="text-xs font-semibold text-base-content/60 uppercase mb-2">{m[\'ai.providers.title\']()}</p>'],
  ['<span class="badge badge-xs badge-primary">default</span>', '<span class="badge badge-xs badge-primary">{m[\'ai.providers.default\']()}</span>'],
  ['<Plus size={14} /> Add Provider', '<Plus size={14} /> {m[\'ai.providers.add\']()}'],
  ['Set as default', "{m['ai.providers.setDefault']()}"],
  ["{savingProvider ? 'Saving…' : m['common.save']()", "{savingProvider ? m['ai.providers.saving']() : m['common.save']()}"],
  ...
  ['Chat with your data, generate schemas, run SQL queries, and search semantically.', "{m['ai.studio.subtitle']()}"],
  ['No AI provider configured.', "{m['ai.studio.noProvider']()}"],
  ['<button class="underline" onclick={() => activeTab = \'settings\'}>Add one here →</button>', '<button class="underline" onclick={() => activeTab = \'settings\'}>{m[\'ai.studio.addProvider\']()}</button>'],
  ...
  ["'Find records where status is pending from the last 7 days'", "m['ai.prompt.pendingRecords']()"],
  ...
  ['<p class="text-xs font-semibold text-base-content/50 uppercase mb-2">Generated SQL</p>', '<p class="text-xs font-semibold text-base-content/50 uppercase mb-2">{m[\'ai.query.generatedSql\']()}</p>'],
  ['{queryResult.data.length} row(s) returned', '{queryResult.data.length} {m[\'ai.query.rowsReturned\']()}'],
  ...
  ['<Plus size={14} /> Create Collection</button>', '<Plus size={14} /> {m[\'ai.action.createCollection\']()}</button>'],
]);
