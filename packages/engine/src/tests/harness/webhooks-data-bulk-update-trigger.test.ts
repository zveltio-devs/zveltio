/**
 * Phase C — WebhookManager.trigger on bulk PATCH update (handlers/bulk.ts afterWrite loop).
 */
...
const COLLECTION = `hwhbulkupd_${Date.now()}`;

d('webhook trigger via bulk update (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  let webhookId = '';
  ...
  beforeAll(async () => {
    ...
    const wh = await app.request('/api/webhooks', {
      ...
      headers: { 'Content-Type': 'application/json', cookie },
      body: JSON.stringify({
        name: `Harness WH bulk upd ${Date.now()}`,
        url: 'https://example.test/hook-bulk-upd',
        events: ['update'],
  ...
  it('PATCH /api/data/:collection/bulk creates delivery rows for each updated record', async () => {
    const res = await app.request(`/api/data/${COLLECTION}/bulk`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', cookie },
      body: JSON.stringify({
        records: [
          { id: id1, title: 'one-upd' },
          { id: id2, title: 'two-upd' },
        ],
      }),
    });
