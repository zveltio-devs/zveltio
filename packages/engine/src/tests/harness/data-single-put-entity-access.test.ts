/**
 * Phase C — PUT replaceRecord entity-access deny (handlers/single.ts replaceRecord).
 */
...
const COLLECTION = `hput_${Date.now()}`;
const OWNER = 'harness-put-entity-access';

d('data single PUT entity access (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  let tableName = '';
  ...
  beforeAll(async () => {
    ...
    const create = await app.request(`/api/data/${COLLECTION}`, {
      ...
      body: JSON.stringify({ title: 'replace-me', amount: 1 }),
    });
  ...
  it('returns 403 on PUT when entity-access denies update', async () => {
    ...
    const res = await app.request(`/api/data/${COLLECTION}/${recordId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', cookie },
      body: JSON.stringify({ title: 'replaced', amount: 99 }),
    });
