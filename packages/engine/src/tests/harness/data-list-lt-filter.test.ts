/**
 * Phase C — LIST lt filter (handlers/list + dynamic.ts buildCondition).
 */
...
const COLLECTION = `hlt_${Date.now()}`;

d('data list lt filter (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  ...
  it('filters with lt on an integer field via bracket syntax', async () => {
    const res = await app.request(`/api/data/${COLLECTION}?score[lt]=50`, {
      headers: { cookie },
    });
    ...
    expect(body.records.some((r) => r.label === 'high')).toBe(false);
  });
});
