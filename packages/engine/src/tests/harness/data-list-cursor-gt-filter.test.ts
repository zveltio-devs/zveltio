/**
 * Phase C — ascending cursor keyset with gt filter (handlers/list.ts).
 */
...
const COLLECTION = `hcgt_${Date.now()}`;

d('data list cursor asc + gt filter (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  ...
  it('paginates ascending with cursor while gt filter is active', async () => {
    const filter = encodeURIComponent(JSON.stringify({ rank: { gt: 10 } }));
    const first = (await (
      await list(`?limit=2&sort=rank&order=asc&filter=${filter}`)
    ).json()) as ListBody;
    expect(first.records.every((r) => r.rank > 10)).toBe(true);
    ...
    expect(page2.records.every((r) => r.rank > 10)).toBe(true);
