/**
 * Phase C — virtual POST success applies column read mask on the proxied record.
 */
...
const COLLECTION = `hvcrmask_${Date.now()}`;
...
d('virtual create response column mask (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  ...
  it('strips unreadable columns from the virtual create response body', async () => {
    originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => ({
      ok: true,
      status: 201,
      json: async () => ({ id: 'ext-9', title: 'ok', secret: 'upstream' }),
      text: async () => '',
    ...
    expect(body.record.title).toBe('ok');
