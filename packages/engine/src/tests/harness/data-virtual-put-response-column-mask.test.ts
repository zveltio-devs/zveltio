/**
 * Phase C — virtual PUT success applies column read mask on the proxied record.
 */
...
d('virtual PUT response column mask (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  ...
  it('strips unreadable columns from the virtual PUT response body', async () => {
    ...
    expect(body.record.title).toBe('replaced');
