/**
 * Phase C — list GET with RLS neq filter (handlers/list.ts RLS branch).
 */
...
d('data list RLS neq (in-process)', () => {
  let app: Hono;
  let db: Database;
  let godCookie = '';
  let memberCookie = '';
  ...
  beforeAll(async () => {
    ...
    const policy = await app.request('/api/admin/rls', {
      ...
      body: JSON.stringify({
        ...
        description: 'hide restricted rows on list',
      }),
    });
  ...
  it('excludes rows hidden by RLS neq on list GET', async () => {
