/**
 * Phase C — WebhookManager.trigger on data PATCH (write-pipeline afterWrite update).
 */
...
const COLLECTION = `hwhupd_${Date.now()}`;

d('webhook trigger via data update (in-process)', () => {
  let app: Hono;
  let db: Database;
  let cookie = '';
  let webhookId = '';
  ...
  beforeAll(async () => {
    ...
    const wh = await app.request('/api/webhooks', {
      ...
      headers: { 'Content-Type': 'application/json', cookie },
      body: JSON.stringify({
        name: `Harness WH update ${Date.now()}`,
        url: 'https://example.test/hook-update',
        events: ['update'],
  ...
  it('PATCH /api/data/:collection/:id creates a delivery row for update', async () => {
