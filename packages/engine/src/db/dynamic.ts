...
export async function dynamicCreateTable(
  ...
  // Standard indexes — CONCURRENTLY avoids ShareLock on table while building
  ...
  await sql`CREATE INDEX CONCURRENTLY IF NOT EXISTS ${idxCreated} ON ${table} (created_at DESC)`.execute(
    db,
  );
  await sql`CREATE INDEX CONCURRENTLY IF NOT EXISTS ${idxStatus}  ON ${table} (status)`.execute(
    db,
  );
}
