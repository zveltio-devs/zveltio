 ...
 * explain, audit-log, stats, logs, slow-queries, field-types, types, api-keys,
 * notifications, revisions). Extracted from admin.ts (H-07 split); registered
 * onto the admin app by adminRoutes(). Route paths are byte-identical.
 */
export function registerSystemRoutes(app: Hono, db: Database): void {
  // ── System Status ─────────────────────────────────────────────
