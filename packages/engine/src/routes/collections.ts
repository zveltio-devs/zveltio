...
export function collectionsRoutes(db: Database, auth: any): Hono {
    ...
    async (c) => {
      ...
      if (colDDL) {
        await db.schema
          ...
          .execute()
          .catch(async () => {
            // Fallback to raw SQL for complex types
            const { sql } = await import('kysely');
            await sql.raw(`ALTER TABLE ${tableName} ADD COLUMN IF NOT EXISTS ${colDDL}`).execute(db);
          });
      }
  ...
  app.delete('/:name/fields/:field', async (c) => {
    ...
    const tableName = DDLManager.getTableName(name);
    const { sql } = await import('kysely');
    await sql.raw(`ALTER TABLE ${tableName} DROP COLUMN IF EXISTS "${fieldName}"`).execute(db);
