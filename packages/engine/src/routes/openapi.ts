...
function buildSpec() {
  return {
    ...
    info: {
      ...
      description: 'Headless BaaS that becomes a self-hosted SaaS through extensions.',
    ...
    paths: {
      ...
      '/collections/{name}': {
        get: {
          ...
          responses: {
            '200': { description: 'Collection schema' },
        ...
        get: {
          ...
          parameters: [
            ...
            },
            {
              ...
              schema: { type: 'string' },
              description:
                'JSON filter: {"field":{"op":value}} — ops: eq, neq, gt, gte, lt, lte, like, in, not_in, null, not_null',
            },
            {
              name: 'field[op]',
              in: 'query',
              schema: { type: 'string' },
              description: 'Bracket filter (alternative to JSON): ?price[gt]=50&title[like]=pro',
            },
            {
              name: 'cursor',
              in: 'query',
              schema: { type: 'string' },
              description: 'Cursor for keyset pagination (base64url from next_cursor)',
            },
            {
              name: 'as_of',
              in: 'query',
              schema: { type: 'string', format: 'date-time' },
              description: 'Time-travel: reconstruct state at this timestamp',
            },
          ],
          responses: {
            '200': {
      ...
      '/data/{collection}/{id}': {
        ...
        patch: {
          ...
          responses: {
            ...
            '403': { description: 'Forbidden — no update policy' },
            '404': { description: 'Not found' },
          },
        },
        delete: {
          ...
          responses: {
            ...
            '403': { description: 'Forbidden — no delete policy' },
            '404': { description: 'Not found' },
          },
        },
      },
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  properties: {
                    subject: { type: 'string', description: 'User ID or role name' },
                    resource: {
                      type: 'string',
                      description:
                        'Collection name (e.g. "products") or system resource (e.g. "admin")',
                    },
                    action: { type: 'string', description: 'read | create | update | delete | *' },
                  },
                },
              },
            },
      ...
      '/permissions/bootstrap': {
        post: {
          ...
          description:
            'Requires `Authorization: Bearer <RECOVERY_TOKEN>` header. Only available when RECOVERY_TOKEN env var is set (min 32 chars).',
          security: [],
          requestBody: {
            required: true,
            content: {
          ...
          responses: {
            '200': { description: 'User promoted to god role' },
            '401': { description: 'Invalid token' },
            '403': { description: 'Recovery mode not enabled' },
      ...
      '/webhooks': {
        ...
        post: {
                ...
                schema: {
                  ...
                  properties: {
                    ...
                    secret: {
                      type: 'string',
                      description:
                        'HMAC-SHA256 signing secret. Omit to auto-generate a 32-byte secret. Plaintext returned only once.',
                    },
                    method: {
                      type: 'string',
          ...
          responses: {
            '201': {
              description: 'Webhook created — `secret` is the plaintext value, shown once only',
            },
          },
        },
      },
      '/webhooks/{id}': {
        get: {
          ...
          responses: {
            '200': { description: 'Webhook object' },
            '404': { description: 'Not found' },
          },
        },
        patch: {
            ...
            '200': {
              ...
              content: {
                'application/json': {
                  schema: {
                    type: 'object',
                    properties: { secret: { type: 'string' }, webhook: { type: 'object' } },
                  },
                },
              },
            },
      ...
      '/api-keys': {
        ...
        post: {
                ...
                schema: {
                  ...
                  properties: {
                    ...
                    scopes: {
                      type: 'array',
                      items: { type: 'string' },
                      description: 'e.g. ["products:read", "orders:create"]',
                    },
          ...
          responses: {
            '201': {
              description:
                'API key created — `key` field contains the `zvk_...` plaintext, shown once only',
            },
          },
        },
      },
          ...
          parameters: [
            {
              ...
              in: 'query',
              schema: { type: 'string' },
              description: 'Comma-separated collection names to watch',
            },
            {
              name: 'channel',
              in: 'query',
              schema: { type: 'string' },
              description: 'Extra channels: broadcast:name, presence:name',
            },
          ],
          responses: {
            '200': {
              description: 'text/event-stream — event types: `data`, `presence`, `broadcast`',
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  description: 'Optional user metadata (display name, avatar, etc.)',
                },
              },
            },
          },
                  ...
                  schema: {
                    ...
                    properties: {
                      ...
                      members: {
                        ...
                        items: {
                          type: 'object',
                          properties: { userId: { type: 'string' }, lastSeen: { type: 'integer' } },
                        },
                      },
                    },
                  },
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  required: ['collection_name', 'column_name', 'role'],
                  properties: {
                    ...
                    role: {
                      type: 'string',
                      description: 'Role name or `*` to match all roles. Admins always bypass.',
                    },
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  properties: { can_read: { type: 'boolean' }, can_write: { type: 'boolean' } },
                },
              },
            },
          },
              ...
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    platform: { type: 'string', enum: ['fcm', 'apns', 'web'] },
                  },
                },
              },
            },
      ...
      '/settings': {
        ...
        patch: {
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: { type: 'object', properties: { key: { type: 'string' }, value: {} } },
              },
            },
          },
