...
const VERSIONS_URL = process.env.ZVELTIO_VERSIONS_URL || 'https://get.zveltio.com/versions.json';
...
export const updateCommand = new Command('update')
  ...
  .action(async (opts) => {
    ...
    const installDir = opts.dir || process.env.ZVELTIO_DIR || join(process.cwd(), 'zveltio');
    ...
    console.log(
      `  Current version: ${c.cyan(`v${currentVersion}`)} ${c.dim(`(from ${versionSource})`)}`,
    );
    ...
    try {
      ...
      console.log('  Options:');
      console.log(
        `    ${c.cyan('zveltio update --version <x.y.z>')}   ${c.dim('skip registry, update to specific version')}`,
      );
      console.log(
        `    ${c.cyan('ZVELTIO_VERSIONS_URL=<url> zveltio update')}   ${c.dim('use a custom registry')}`,
      );
      console.log('');
      process.exit(1);
    }
    ...
    if (opts.check) {
      ...
      if (hasUpdate) {
        ...
        if (entry?.breaking_changes) {
          console.log(
            c.yellow('  ⚠  Contains BREAKING CHANGES — read release notes before updating.'),
          );
        }
