...
| S5-05 | WASM sandbox for third-party extensions (Wasmtime) | 5 | 2w | DONE-FOUNDATION — `lib/extension-sandbox.ts` ships the policy/quota/observability layer the future WASM sandbox will plug into. Closed enum of capabilities (`db.read/write`, `fetch.http/https`, `crypto.subtle`, `env.read`, `fs.read/write`, `process.spawn`). First-party extensions (detected by name prefix against the `zveltio-extensions/` taxonomy) get the full set minus `process.spawn`; third-party get tighter defaults (HTTPS-only, no env, no fs, bounded CPU + memory). `EXTENSION_POLICIES_JSON` env override per-extension. `policyFor(extName)` + `hasCapability(extName, cap)` are the public entry points; cached per process. Telemetry hook `observePolicyDecision` emits engine events on every denial — same surface a future WASM enforcer would call. 16 unit tests pin first-party-vs-third-party detection (no substring matching), default capability sets, quotas split, cache stability. **Full WASM isolation** (Wasmtime / Wasmer + capability bridge for db/fetch/crypto + build pipeline) defers to v2.0 once a third-party ecosystem actually exists; today every shipped extension is first-party. |
...
| S5-07 | Electric SQL offline sync in SDK | 5 | 2w | DONE-FOUNDATION — new `@zveltio/sdk/offline` subpath exposes `createOfflineProvider(config)` with a typed `OfflineProvider` interface (pull / push / subscribe / close). Two strategies selectable at construction: `'crdt'` (default) adapts the existing `SyncManager` / `LocalStore` / `mergeLWW` path; `'electric'` is a stub that throws `ElectricNotConfigured` until the operator stands up an Electric service and wires the proxy URL. The API shape is the long-term commitment — applications written today migrate to real Electric tomorrow by flipping one field, no data-layer rewrite. 7 unit tests cover construction for both kinds, stub-throws semantics, error-message guidance (points users at `provider: 'crdt'` as the working alternative). Real Electric wiring (replica-identity ALTERs, proxy-URL JWT handshake, electric-sql client integration) lands in a follow-up wave once there's an actual deployment to test against. |
