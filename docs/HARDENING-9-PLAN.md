...
| H-07 | Split `routes/admin.ts` and the collections detail Svelte page | 2 | 1.5d | **ENGINE HALF DONE** (2188981, 1/2): `routes/admin.ts` **1347→244 L** — `adminRoutes()` body cut along its section seams into `routes/admin/{system-routes(565),permission-routes(310),config-routes(299)}.ts` (register-fn pattern, same call order → byte-identical paths). All <600. Pure move: markers conserved 26→26, routes 45→45. Verified: tsc 0, WSL 404/0, engine boot mounts every group (/status,/roles,/rate-limits → 401), gates green. **STUDIO HALF PENDING (2/2):** extract `CollectionDataTable`/`FieldEditor`/`RelationPanel` from `collections/[name]/+page.svelte` (1678 L, 787 script + 888 markup, 4 tabs). RISK: Svelte 5 cross-tab runes state; the only automated net is svelte-check (baseline **22 pre-existing errors** in other pages; catches types, NOT reactivity breaks) — needs the studio dev server + preview for a real runtime check. Do as a focused session with the preview running. |
