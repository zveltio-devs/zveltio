...
| H-11 | Upgrade-path test in CI (release N-1 binary → HEAD migration → smoke) | 3 | 1d | **DONE** (PR #25) — new `.github/workflows/upgrade-path.yml` (on migration/engine PRs + nightly + dispatch): boots the latest published release binary, seeds a collection (text/number/boolean/json/date) + records with sentinels + a webhook + a flow via the public API, stops it, builds HEAD, boots HEAD against the SAME Postgres (auto-migrate), then asserts every seeded value reads back byte-for-byte, the auth session still signs in, and `/api/health` is green. Reverse guard: HEAD's migration `sql/` must be a strict superset of the release tag's (filename + sha256) — fails on any rename/renumber/delete. Reusable `scripts/upgrade-smoke.ts` (seed/verify, HTTP-only; seed records what N-1 actually returns so serialization coercion isn't a false positive). Runbook note added to `docs/platform/disaster-recovery.md` Scenario A. Verified locally (WSL+PG, non-superuser role): seed→verify green end-to-end; superset guard green vs `v3.0.0-beta.29`. |
...
2. Write a one-shot codemod script (`scripts/suppress-existing-any.ts`) that
   inserts `// biome-ignore lint/suspicious/noExplicitAny: legacy — see docs/private/HARDENING-9-PLAN.md H-01`
...
3. Add `scripts/any-ratchet.ts`: counts `biome-ignore.*noExplicitAny` across
   ...
   (`quality-gates/any-baseline.json`, per-package counts). Fails if any count
...
`scripts/any-ratchet.ts` (new), `quality-gates/any-baseline.json` (new),
...
2. Add `scripts/coverage-gate.ts`: parses the summary, compares line coverage
   ...
   committed baseline (`quality-gates/coverage-baseline.json`). Fail on drop
...
`scripts/coverage-gate.ts` (new), `quality-gates/coverage-baseline.json` (new),
...
- **(b) Don't**: add a "Supported platforms" section to `README.md` and
  `docs/platform/security-model.md` stating Linux/macOS/WSL only, and make `install/install.sh`
...
- [x] Platform policy stated unambiguously — no "third state". Resolved via the
      **soft-docs** branch (owner chose this over both a Windows CI job and a
      runtime nag): `README.md` "Supported platforms" matrix + `docs/platform/security-model.md`
...
- [ ] Runbook note added to `docs/platform/disaster-recovery.md` cross-referencing it.
...
- [ ] Existing bundled extensions (backup, saved-queries, schema-branches,
      ...
      own changelog line.
- [ ] `docs/extensions/developer-guide.md` updated (tenancy section).
...
- [ ] This is a **breaking-ish** change: gate behind beta, note in CHANGELOG
      and `docs/platform/migration-alpha-to-beta.md` successor doc.
...
`docs/platform/versioning.md` (document the gate).

**Acceptance criteria.**
- [ ] Gate runs in CI on release tags and blocks a stable tag when any check
      fails; prerelease tags (`-beta.*`) bypass with a warning.
- [ ] `docs/platform/versioning.md` states: version numbers are never renumbered again;
