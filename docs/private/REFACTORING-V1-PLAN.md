...
| S3-03 | `registerSlot` for Studio composition | 3 | 1d | DONE — `<Slot name="..." ctx={{...}}>` consumer + reactive registry in `extension-api.svelte.ts`. **Historical:** v1 used runtime `bundle-loader.ts` (alpha.93–beta.14, removed beta.15). **Current (Model 2.5):** extensions ship `studio/src/contribute.ts`; `sync-extensions.ts` copies it to `$lib/ext/<name>/` and the admin layout calls `activate()` for enabled extensions via `$lib/ext/.contributions.generated.ts`. Owner-aware `registerContributionSlot` / `unregisterContributions` on disable. Demo slot `dashboard.widgets` on admin home; CRM receivables pilot. Sort logic tested (6+ unit tests in `studio-extension-api.test.ts`). |
