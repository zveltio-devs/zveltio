#!/usr/bin/env bun
/**
 * `noExplicitAny` suppression ratchet — hardening plan item H-01.
 *
...
function writeBaseline(counts: Record<string, number>, total: number): void {
  ...
  const baseline: Baseline = {
    ...
    note: 'Per-bucket noExplicitAny suppression counts. Ratchet: counts may only decrease. See hardening plan item H-01.',
    total,
    counts: sorted,
  };
